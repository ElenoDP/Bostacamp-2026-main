...
class Janela:
    def __init__(self):
        ...
        self.chamar_proxima.pack()

        self.rechamar = tk.Button(self.janela, text="Rechamar paciente", command=self.rechamar_paciente)
        self.rechamar.pack()

        self.registrar_evasao = tk.Button(self.janela, text="Registrar Evasão", command=self.registrar_evasao)
        self.registrar_evasao.pack()

        tk.Button(self.janela, text="Abrir Visor do Paciente", command=self.abrir_visor_paciente).pack(pady=20)
    ...
    def cadastrar_nome(self):
        ...
        if self.sistema.cadastrar_paciente(nome):
    ...
    def chamar_proximo_nome(self):
        nome = self.sistema.chamar_proximo_paciente()
    ...
    def rechamar_paciente(self):
        nome = self.sistema.ultimo_chamado

        if nome:
            if self.sistema.rechamar_paciente(nome):
            ...
            messagebox.showinfo("Informação", "Não há paciente para rechamar.")
